...
private:
    ...
    long mSimSensorStatus = 0;
    long mFullySensorStatus = 0;
    const long CLOSE_SENSOR_DEVICE_ID = 7;
    const long FULLY_SENSOR_DEVICE_ID = 6;
	/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
    ...
    , mDOMPrintableKeyValue(0)
{
    ...
    //     input character.
	/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
    LOG("bandit:KeyEventDispatcher get mData.key.keyCode %d, mData.key.scanCode %d", mData.key.keyCode, mData.key.scanCode);
    /*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
...
KeyEventDispatcher::Dispatch()
{
    ...
    //which:
    //0 means close,1 means sim open,2means fully open
    // SLIDER_REPORT_SCAN_CODE = 249;
    if (mData.key.scanCode == 249){
        long status = 0;
        long SLIDE_CLOSE_STATUS = 0;
        long SLIDE_SIM_OPEN_STATUS = 1;
        long SLIDE_FULLY_OPEN_STATUS = 2;

        //this is sim sensor device event
        if(mData.deviceId == CLOSE_SENSOR_DEVICE_ID) {
            if(mData.action == AKEY_EVENT_ACTION_DOWN)
            {
                LOG("bandit:deviceId = CLOSE_SENSOR_DEVICE_ID ; AKEY_EVENT_ACTION_DOWN status = SLIDE_CLOSE_STATUS");
                status = SLIDE_CLOSE_STATUS;
            }
            if(mData.action == AKEY_EVENT_ACTION_UP)
            {
                LOG("bandit:deviceId = CLOSE_SENSOR_DEVICE_ID ; AKEY_EVENT_ACTION_UP status = SLIDE_SIM_OPEN_STATUS");
                status = SLIDE_SIM_OPEN_STATUS;
            }
        }
        //this is fully sensor device event
        if(mData.deviceId == FULLY_SENSOR_DEVICE_ID) {
            if(mData.action == AKEY_EVENT_ACTION_DOWN)
            {
                LOG("bandit:deviceId = FULLY_SENSOR_DEVICE_ID ; AKEY_EVENT_ACTION_DOWN status = SLIDE_FULLY_OPEN_STATUS");
                status = SLIDE_FULLY_OPEN_STATUS;
            }
            if(mData.action == AKEY_EVENT_ACTION_UP)
            {
                LOG("bandit:deviceId = FULLY_SENSOR_DEVICE_ID ; AKEY_EVENT_ACTION_UP status = SLIDE_SIM_OPEN_STATUS");
                status = SLIDE_SIM_OPEN_STATUS;
            }
        }
