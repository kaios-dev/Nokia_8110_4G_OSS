...
Cu.import('resource://gre/modules/kaios_service.jsm');
...
// Added for Task5233386 Sep 7 2017 Begin
// Added for Bug 206 Oct 17 2017 Begin
// ================== SMS ==================
/* << [ARG-71]: BDC kanxj 201901022 Cyrillic characters are not displayed in SMS in case of reduced character set is selected >> */
/*SettingsListener.observe('ril.sms.encoding_mode', 0 , function(value) {
  dump("detect ril.sms.encoding_mode changed:" + value);
  if (value == 1) {
    libcutils.property_set('persist.sms.strict7BitEncoding', 'false');
  } else {
    libcutils.property_set('persist.sms.strict7BitEncoding', 'true');
  }
});*/
// Added for Bug 206 Oct 17 2017 End
// Added for Task5233386 Sep 7 2017 End

/* Added for Task5229893 20170921 lina.zhang@t2mobile.com -begin */
SettingsListener.observe('two_digit_num_dial.enable', false , function(value) {
  dump("detect two_digit_num_dial.enable changed:" + value + ', type = ' + typeof(value));
  if (value || value == 'true') {
    libcutils.property_set('persist.two_digit_dial.enable', 'true');
  } else {
    libcutils.property_set('persist.two_digit_dial.enable', 'false');
  }
});
/* Added for Task5229893 20170921 lina.zhang@t2mobile.com -end */
...
(function DeviceInfoToSettings() {
  ...
  if (isGonk) {
    ...
    // Added for Bug763 for feedback by lina.zhang@t2mobile.com 20111108 begin.
    release_version = libcutils.property_get('ro.build.version.fih');
    dump('customization settings.js release_version = ' + release_version);
    // Added for Bug763 for feedback by lina.zhang@t2mobile.com 20111108 end.
  ...
  req.onsuccess = req.onerror = () => {
    ...
    let setting = {
      ...
      'deviceinfo.release_version': release_version,
