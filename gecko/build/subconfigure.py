...
def run(objdir):
    ...
    if not skip_config_status:
        if skip_configure:
            ...
            sys.stdout.flush()
        try:
            #modify by minzhang@t2mobile.com for task 5401635 start
            #The config.status is a file in python format,the info of the head is too long to execute
            # ./config.status failed,but can excute "python ./config.status".
            #Temp plan:avoid excute ./config.status while the config.status is a file in python format.
            ...
            if mozpath.normsep(relobjdir) == 'js/src':
                ...
                output += subprocess.check_output([data['shell'], '-c',
                    './config.status'], stderr=subprocess.STDOUT, cwd=objdir,
                    env=data['env'])
            #modify by minzhang@t2mobile.com for task 5401635 end
        except subprocess.CalledProcessError as e:
            ret = e.returncode
            output += e.output
