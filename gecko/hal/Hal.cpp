...
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
class SlideObserversManager : public ObserversManager<long>
{
protected:
  void EnableNotifications() {
    PROXY_IF_SANDBOXED(EnableSlideNotifications());
  }

  void DisableNotifications() {
    PROXY_IF_SANDBOXED(DisableSlideNotifications());
  }
};

static SlideObserversManager sSlideObservers;

void
RegisterSlideObserver(SlideObserver* aObserver)
{
  AssertMainThread();
  sSlideObservers.AddObserver(aObserver);
}
...
void
UnregisterSlideObserver(SlideObserver* aObserver)
{
  AssertMainThread();
  sSlideObservers.RemoveObserver(aObserver);
}

void
UpdateSlideState(const long& aSlideState)
{
  LOG("UpdateSlideState ENTER");
  AssertMainThread();
  LOG("UpdateSlideState BroadcastInformation");
  sSlideObservers.BroadcastInformation(aSlideState);
}

void
NotifySlideStateFromInputDevice(long aSlideState)
{
  LOG("NotifySlideStateFromInputDevice enter");
  AssertMainThread();
  LOG("NotifySlideStateFromInputDevice notify");
  PROXY_IF_SANDBOXED(NotifySlideStateFromInputDevice(aSlideState));
}
...
{
  AssertMainThread();
  PROXY_IF_SANDBOXED(RequestCurrentSlideState());
}
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
void
...
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
long GetCurrentSlideStatus()
{
  AssertMainThread();
  RETURN_PROXY_IF_SANDBOXED(GetCurrentSlideStatus(), 0);
}
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
