...
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
#undef LOG
#define LOG(...) do{\
__android_log_print(ANDROID_LOG_DEBUG, "GonkHal", __VA_ARGS__);\
__android_log_print(ANDROID_LOG_DEBUG, "GonkHal", ":--(%s:%d) %s: ",__FILE__, __LINE__, __func__);\
}while(0)
...
AdjustSystemClock(int64_t aDeltaMilliseconds)
{
  ...
  char useTimerfd[PROPERTY_VALUE_MAX];
  property_get("ro.kaios.use_timerfd", useTimerfd, "");
...
EnableAlarm()
{
  ...
  if (strcmp(useTimerfd, "true") == 0) {
    alarmFd = timerfd_create(CLOCK_POWEROFF_ALARM, 0);
    if (alarmFd < 0) {
...
SetAlarm(int32_t aSeconds, int32_t aNanoseconds)
{
  ...
  if (strcmp(useTimerfd, "true") == 0) {
    ...
    result = timerfd_settime(sAlarmData->mFd, TFD_TIMER_ABSTIME, &ts, NULL);
  } else {
    struct timespec ts;
    ts.tv_sec = aSeconds;
...
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
long
GetCurrentSlideStatus()
{
  //status 99 main error
  int slideEvent1Status = 99;
  int slideEvent2Status = 99;
  bool success = false;
  LOG("bandit GonkHal GetCurrentSlideStatus ENTER");

  success = ReadSysFile("/sys/class/input/event1/device/device/hall_status", &slideEvent1Status);
  if (!success) {
    return 99;
  }
  success = false;
  success = ReadSysFile("/sys/class/input/event2/device/device/hall_status", &slideEvent2Status);
  if (!success) {
    return 99;
  }
  //if event1 = 1,event2 = 0 that means current status is close
  //if event1 = 0,event2 = 0 that means current status is sim open
  //if event1 = 0,event2 = 1 that means current status is fully open
  //if event1 = 1 event2 = 1 that means error status 99
  LOG("bandit GonkHal GetCurrentSlideStatus slideEvent1Status = %d",slideEvent1Status);
  LOG("bandit GonkHal GetCurrentSlideStatus slideEvent2Status = %d",slideEvent2Status);
  if((slideEvent1Status == 1) && (slideEvent2Status == 0)) {
    return 0;
  } else if ((slideEvent1Status == 0) && (slideEvent2Status == 0)) {
    return 1;
  } else if ((slideEvent1Status == 0) && (slideEvent2Status == 1)) {
    return 2;
  }else if ((slideEvent1Status == 1) && (slideEvent2Status == 1)) {
    return 99;
  }
...
void
NotifySlideStateFromInputDevice(long aSlideState)
{
  LOG("bandit GonkHal NotifySlideStateFromInputDevice ENTER");
  hal::UpdateSlideState(aSlideState);
}

void
RequestCurrentSlideState()
{
  long SlideState = GetCurrentSlideStatus();
  LOG("bandit GonkHal RequestCurrentSlideState ENTER");
  //todo should get current slide status
  hal::UpdateSlideState(SlideState);
}

void
EnableSlideNotifications()
{
}

void
DisableSlideNotifications()
{
}
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
} // hal_impl
