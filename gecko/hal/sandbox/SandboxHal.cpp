...
void
NotifySlideStateFromInputDevice(long aSlideStatus)
{
}
...
{
  Hal()->SendEnableSlideNotifications();
}

void
DisableSlideNotifications()
{
  Hal()->SendDisableSlideNotifications();
}

void
RequestCurrentSlideState()
{
  Hal()->SendGetSlideState();
}
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
void
...
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
  virtual bool
  RecvEnableSlideNotifications() override
  {
  ...
  virtual bool
  RecvDisableSlideNotifications() override
  {
    hal::UnregisterSlideObserver(this);
    return true;
  }
  ...
  virtual bool
  RecvGetSlideState() override
  {
    long slideState = hal::GetCurrentSlideStatus();
    Unused << SendNotifySlideState(slideState);
    return true;
  }

  void Notify(const long& aSlideState)
  {
    Unused << SendNotifySlideState(aSlideState);
  }
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
...
public:
  ...
  /*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
  virtual bool
  RecvNotifySlideState(const long& aSlideState) override{
...
{
  NS_RUNTIMEABORT("GetCurrentSlideStatus() can't be called from sandboxed contexts.");
  return 0;
}
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
} // namespace hal_sandbox
