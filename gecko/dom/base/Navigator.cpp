...
NS_IMPL_CYCLE_COLLECTION_TRAVERSE_BEGIN(Navigator)
  ...
  /*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
  NS_IMPL_CYCLE_COLLECTION_TRAVERSE(mSlideManager)
...
Navigator::Invalidate()
{
  ...
  /*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
  if (mSlideManager) {
    mSlideManager->Shutdown();
    mSlideManager = nullptr;
  }
...
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
long
Navigator::GetSlideStatus(ErrorResult& aRv)
{
	long cs = mSlideManager->CurrentSlideStatus();
  return cs;
}
...
/*add by xuejiang.liu for Argon Slider Feature for task:5218350*/
already_AddRefed<Promise>
Navigator::GetSlideManager(ErrorResult& aRv)
{
  if (!mWindow || !mWindow->GetDocShell()) {
    aRv.Throw(NS_ERROR_UNEXPECTED);
    return nullptr;
  ...
  if (!mSlideManager) {
    mSlideManager = new SlideManager(mWindow);
    mSlideManager->Init();
  }

  RefPtr<Promise> p = mSlideManager->GetPromise(aRv);
  return p.forget();
}
...
Navigator::GetFeature(const nsAString& aName, ErrorResult& aRv)
{
  ...
  if (aName.EqualsLiteral("kaios.api_daemon.version")) {
    nsCString propertyKey("kaios.api_daemon.version");
    char value[PROPERTY_VALUE_MAX];
    uint32_t len = property_get(propertyKey.get(), value, nullptr);
    if (len > 0) {
      p->MaybeResolve(NS_ConvertUTF8toUTF16(value));
