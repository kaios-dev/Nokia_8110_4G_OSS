...
var WifiManager = (function() {
  ...
  //[task-5206929] modify by shangpeng.su 2017.08.23 begin
  manager.networkConfiguratedFromDOM = function (networkConfigurated, mozWifiNetwork) {
    var self = this;

    function hasValidProperty(name) {
      return ((name in mozWifiNetwork) && mozWifiNetwork[name] != null);
    }

    for (var i = 0; i < networkConfigurationFields.length; i++) {
      var fieldName = networkConfigurationFields[i].name;
      if (fieldName in mozWifiNetwork) {
        if (mozWifiNetwork[fieldName] !== null && mozWifiNetwork[fieldName] !== 0) {
          networkConfigurated[fieldName] = mozWifiNetwork[fieldName];
        }
      } else {
        if (hasValidProperty("keyManagement")) {
          networkConfigurated.key_mgmt = mozWifiNetwork.keyManagement;
          networkConfigurated.key_mgmt += " IEEE8021X";
        }
        if (hasValidProperty("subjectMatch")) {
          networkConfigurated.subject_match = mozWifiNetwork.subjectMatch;
        }
        if (hasValidProperty("serverCertificate")) {
          networkConfigurated.ca_cert =
                  quote("keystore://WIFI_SERVERCERT_" + mozWifiNetwork.serverCertificate);
        }
      }
    }
  }
  //[task-5206929] modify end
...
function WifiWorker() {
  ...
  const messages = ["WifiManager:getNetworks", "WifiManager:getKnownNetworks",
                    ...
                    "WifiManager:saveNetworkConfigurated",
...
WifiWorker.prototype = {
  ...
  receiveMessage: function MessageManager_receiveMessage(aMessage) {
    ...
    switch (aMessage.name) {
      ...
      case "WifiManager:setWifiTethering":
        ...
        break;
      //[task-5206929] modify by shangpeng.su 2017.08.23 begin
      case "WifiManager:saveNetworkConfigurated":
        this.saveNetworkConfigurated(msg);
        break;
      //[task-5206929] modify end
  ...
  //[task-5206929] modify by shangpeng.su 2017.08.23 begin
  saveNetworkConfigurated: function(msg) {
    const message = "WifiManager:saveNetworkConfigurated:Return";
    var self = this;
    ...
    var network = msg.data;
    var networkKey = getNetworkKey(network);
    var networkConfigurated = {};
    WifiManager.networkConfiguratedFromDOM(networkConfigurated, network);

    dump("do saveNetworkConfigurated in wifiworker, network ssid=" + networkConfigurated.ssid);

    WifiManager.addNetwork(networkConfigurated, function(ok){
      if (!ok) {
        self._sendMessage(message, false,
          "saveNetworkConfigurated failed due setNetworkConfiguration", msg);
        return;
      }
      ...
      WifiManager.saveConfig(function(ok){
          if (ok) {
            ...
            self._sendMessage(message, true,
              "saveNetworkConfigurated " + networkConfigurated.ssid + " success.", msg);
          } else {
            self._sendMessage(message, false,
              "saveNetworkConfigurated failed due saveConfig", msg);
          }
      }.bind(this));
    }.bind(this));
  },
