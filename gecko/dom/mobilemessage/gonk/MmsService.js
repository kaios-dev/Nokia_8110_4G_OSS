...
/* Added for BUG 2427 Jan 10 2018 Begin */
function isDataConnected(aServiceId) {
  var allowMmsWhenDataOff = true;
  try {
    allowMmsWhenDataOff = libcutils.property_get("dom.mms.service_mode", "true") === "true";
  } catch (e) {
    debug("MyTag read prop failed:" + e.stack);
  }
  debug("MyTag @isDataConnected allowMmsWhenDataOff:" + allowMmsWhenDataOff);
  let connection = gMobileConnectionService.getItemByServiceId(aServiceId);
  if (connection.data) {
      debug("MyTag connection existed in RIL_:" + aServiceId);
      debug("MyTag connection connected:" + connection.data.connected);
  }
  return connection.data.connected || allowMmsWhenDataOff;
}
/* Added for BUG 2427 Jan 10 2018 End */
...
MmsConnection.prototype = {
  ...
  onDisconnectTimerTimeout: function() {
    ...
    let deactivateMmsDataCall = (aError) => {
      ...
      if (DEBUG) debug("deactivateDataCallByType NETWORK_TYPE_MOBILE_MMS");
  ...
  */
  ...
  acquire: function(callback) {
    ...
    if (!this.connected) {
      ...
      } else if (!isDataConnected(this.serviceId)) {
        /* Added for BUG 2427 Jan 10 2018 Begin */
        debug("MyTag MMS service not allowed when data is off");
        errorStatus = _HTTP_STATUS_ACQUIRE_TIMEOUT;
        /* Added for BUG 2427 Jan 10 2018 End */
      }
      if (errorStatus != null) {
        if (DEBUG) debug("to call this.flushPendingCallbacks, error status: " + errorStatus );
      ...
      try {
        let allowToAcquire = libcutils.property_get("persist.mms.allowed","false") == "true";
        if (allowToAcquire) {
          this.radioInterface.setupDataCallByType(Ci.nsINetworkInfo.NETWORK_TYPE_MOBILE_MMS);
        } else {
          debug("persist.mms.allowed: false");
          errorStatus = _HTTP_STATUS_ACQUIRE_TIMEOUT;
          this.flushPendingCallbacks(errorStatus);
          this.connectTimer.cancel();
        }
      } catch (e) {
        debug("setupDataCallByType failure: since APN is empty, then failed to setup DataCall.");
        errorStatus = _HTTP_STATUS_ACQUIRE_TIMEOUT;
        this.flushPendingCallbacks(errorStatus);
        this.connectTimer.cancel();
      }
   ...
   */
  release: function() {
    if (DEBUG) debug("release with this.refCount:"+this.refCount);
    this.refCount--;
    if (this.refCount <= 0) {
      this.refCount = 0;
  ...
  ensureRouting: function(url) {
    ...
    return gNetworkManager.addHostRoute(this.networkInfo, host)
      .then(() => {
        if (DEBUG) debug("addHostRoute call back");
        if (this.hostsToRoute.indexOf(host) < 0) {
          this.hostsToRoute.push(host);
          if (DEBUG) debug("addHostRoute this.hostsToRoute:"+JSON.stringify(this.hostsToRoute));
        }
      });
  },
  ...
  observe: function(subject, topic, data) {
    switch (topic) {
      case kNetworkConnStateChangedTopic: {
        ...
        if (DEBUG) debug("network state change connected: "+ connected);
        if (DEBUG) debug("network state change this.connected: "+ this.connected);
  ...
  let helper = {
    ...
    sendHttpRequest: function(mmsConnection, method, url, istream, proxyFilter,
      ...
      try {
        ...
        // Bug1616-tingtingchao@t2mobile.com-begin
        // Defect4245-tingting.chao@t2mobile.com-change mms UA profile-begin
        //uaProfUrl ="http://useragentprofile.hmdglobal.com/uaprof/Nokia33103Gr100.xml";
        uaProfUrl ="http://useragentprofile.hmdglobal.com/uaprof/Nokia81104Gr100.xml";
        // Defect4245-tingting.chao@t2mobile.com-change mms UA profile-end

        // Bug1616-tingtingchao@t2mobile.com-end
        if (uaProfUrl) {
          xhr.setRequestHeader(uaProfTagname, uaProfUrl);
          if (DEBUG) debug("uaProfUrl ="+uaProfUrl);
        }
        // Task5216730-tingting.chao@t2mobile.com-add for mms user agent-begin
        // Defect4245-tingting.chao@t2mobile.com-change mms user agent-begin
        //var user_agent_name = "Nokia3310-MMS/2.0";
        var user_agent_name = "Nokia8110_4G-MMS/1.1;KaiOS 2.5";
        // Defect4245-tingting.chao@t2mobile.com-change mms user agent-end
        xhr.setRequestHeader("User-Agent", user_agent_name);
        // Task5216730-tingting.chao@t2mobile.com-add for mms user agent-end
  ...
  run: {
    value: function(callback) {
      ...
      let retryCallback = (mmsStatus, msg) => {
        if (DEBUG) debug("retryCallback1 mmsStatus: " + mmsStatus);
...
function SendTransaction(mmsConnection, cancellableId, msg, requestDeliveryReport) {
  ...
  if (!msg.headers["x-mms-transaction-id"]) {
    ...
    msg.headers["x-mms-transaction-id"] = "T" + Math.floor(Math.random()*100000000000).toString();
  }
...
SendTransaction.prototype = Object.create(CancellableTransaction.prototype, {
  ...
  run: {
    value: function(callback) {
      ...
      if (!this.istream) {
        this.runCallbackIfValid(MMS.MMS_PDU_ERROR_TRANSIENT_FAILURE, null);
        return;
      }

      this.retryCount = 0;
      let retryCallback = (mmsStatus, msg) => {
        if (DEBUG) debug("retryCallback2 mmsStatus: " + mmsStatus);
...
MmsService.prototype = {
  ...
  handleNotificationIndication: function(serviceId, notification) {
    ...
    gMobileMessageDatabaseService
      ...
      try {
        ...
        if (DEBUG) debug(`retrievalMode is ${retrievalMode}`);
        //// Bug527-gang-chen@t2mobile.com-retrieval mode-begin
        //let tempRetrievalMode = Services.prefs.getCharPref(kPrefRetrievalMode);
        //if (tempRetrievalMode == '1') {
            //retrievalMode = RETRIEVAL_MODE_MANUAL;
        //} else if (tempRetrievalMode == '0') {
            //retrievalMode = RETRIEVAL_MODE_AUTOMATIC_HOME;
        //}
        //// Bug527-gang-chen@t2mobile.com-retrieval mode-end
      } catch (e) {}
  ...
  send: function(aServiceId, aParams, aRequest) {
    ...
    gMobileMessageDatabaseService
      ...
      /* Added for BUG 2427 Jan 10 2018 Begin */
      if (!isDataConnected(aServiceId)) {
          debug("My Tag Error! APN is not ready when sending MMS.");
          sendTransactionCb(mmsMessage,
                            Ci.nsIMobileMessageCallback.SERVICE_NOT_AVAILABLE_ERROR,
                            null);
        return;
      }
      /* Added for BUG 2427 Jan 10 2018 End */
  ...
  retrieve: function(aMessageId, aRequest) {
    ...
    gMobileMessageDatabaseService
      ...
      /* Added for BUG 2427 Jan 10 2018 Begin */
      if (!isDataAllowed) {
        debug("MyTag Error! MMS APN not allowed when data is off.");
        aRequest.notifyGetMessageFailed(
          Ci.nsIMobileMessageCallback.SERVICE_NOT_AVAILABLE_ERROR);
        return;
      }
