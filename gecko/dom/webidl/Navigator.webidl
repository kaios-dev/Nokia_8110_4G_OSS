...
partial interface Navigator {
  [Throws, Pref="dom.slide.enabled", CheckAnyPermissions="slide", AvailableIn=CertifiedApps]
  Promise<SlideManager> getSlideManager();
};
...
partial interface Navigator {
  // This is a safety catch for System App.
  // Return the current slide status. This attribute is only available for apps
  // who runs on the main process, and it should be accessed by System App only.
  [Throws, Pref="dom.slide.enabled", CheckAnyPermissions="slide", AvailableIn=CertifiedApps]
  readonly attribute long SlideStatus;
};
